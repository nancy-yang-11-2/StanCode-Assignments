...
def get_y_coordinate(height, ranking):
    """
    Input:
        height: The height of the canvas.
        ranking: The name's ranking of the year.
    Returns:
        y_coordinate: The y coordinate of the associated text on the vertical line of the year.
    """
    line_gap = (height - 2 * GRAPH_MARGIN_SIZE) / MAX_RANK
    y_coordinate = GRAPH_MARGIN_SIZE + (ranking * line_gap)
    return y_coordinate
...
def get_x_coordinate(width, year_index):
    ...
    x_coordinate = GRAPH_MARGIN_SIZE + line_gap * year_index
    return x_coordinate
    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(rank)):
            if int(rank[j]) <= 1000:
                ...
                line_position.append((x, y))
        canvas.create_line(line_position, width=LINE_WIDTH, fill=color)
